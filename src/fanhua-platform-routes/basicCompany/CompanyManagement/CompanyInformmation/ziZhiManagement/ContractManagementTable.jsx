...
import { Card, Row, Col, Tabs, Icon, Button, Radio, Select,Table,Switch,Modal , message,Popconfirm} from 'antd';
...
function onChange(status,id) {
  ...
  RequireUtils.baseRequire( "company/check-cert",{id:id,cert_flag:cert_flag},function (data) {
    ...
    const columns = [{
    ...
    },
      {
        ...
        render: (text, record) => {
          ...
          else if (parseInt(record.cert_flag)===1){
            return <span>
                 <a href="javaScript:void(0)" style={{marginRight:'10px'}}>&nbsp;&nbsp;
                   <Popconfirm placement="top" title={"Are you sure delete this task?"} onConfirm={()=>(onChange.bind(this)("yes", record.id))} okText="Yes" cancelText="No">
                   <span >通过</span>
                  </Popconfirm>
                  </a>
            <a href="javaScript:void(0)" style={{marginRight:'10px'}}>&nbsp;&nbsp;
              <Popconfirm placement="top" title={"Are you sure delete this task?"} onConfirm={()=>(onChange.bind(this)("no"  ,record.id))} okText="Yes" cancelText="No">
                   <span >不通过</span>
                  </Popconfirm>
                  </a>
      ...
      <div className={'my-table-title-center'}>
        ...
        <Modal title="上传资质"
          ...
          <AddContractManagement
            ...
            go={()=>this.fetch({...this.state.pagination,id:localStorage.getItem("id")})}
            id={id}
            onRef={this.onRef}
          />
        </Modal>
