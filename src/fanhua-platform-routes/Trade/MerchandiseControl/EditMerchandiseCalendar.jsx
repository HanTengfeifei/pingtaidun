...
console.log(price_date);
    ...
    for (var i = 1; i < price_date.length - 1; i++) {
      if (price_date[i -1].goods_date.replace(/-/g,"/") <= value.format("YYYY-MM-DD").replace(/-/g,"/") < price_date[i].goods_date.replace(/-/g,"/")) {
    ...
    return(
      <div>
        {editable
          ...
          : <div><div>{value}</div>
            <div  style={{ padding:"0,0" , margin: '-5px 0',width:88 ,height:116}} onClick={this.switch}> </div></div>
        }
      </div>
    )
  }
