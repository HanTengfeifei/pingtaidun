...
function comfirmUpdateOrderOp(e,id,_pmyself,go) {
  ...
  e.preventDefault();
  function callback(){
    RequireUtils.baseRequire('/order/confirm-modify',{id:id},function (data) {
      if(data.code==1) {
        alert(data.msg);
        go.bind(_pmyself)() ;
      }else{
  ...
  firstComfirm('您确定要修改该订单吗？','确认后，将不可更改',callback)
}
...
function giveUpUpdateOrderOp(e,id,_pmyself,go) {
  ...
  firstComfirm('您确定要放弃修改该订单吗？','确认后，将不可更改',callback)
}
function paymentOp(e,id,_pmyself,go) {
  ...
  firstComfirm('您确定要支付该订单吗？','确认后，将不可更改',callback)
}
...
function giveUpCancelOp(e,id,_pmyself,go) {
  ...
  firstComfirm('您确定要取消该订单吗？','确认后，将不可更改',callback)
}
...
function updateOrderConfirmOp(e,id,_pmyself,go) {
  ...
  function callback(){
    RequireUtils.baseRequire('order/submit-modify',{id:id,new_target:JSON.stringify(need_sumbmit)},function (data) {
      if(data.code==1) {
        alert(data.msg);
        go.bind(_pmyself)() ;
      }else{
...
function updateBalanceOp(e,id) {
  createHistory().push({
    pathname: '/balacedetaillist/?orderid='+id
  })
}
...
function getSaleButton(record,_pmyself,go) {
  ...
  const alertPayment =  <span className={'my-custom-center'} style={{marginBottom:'3px'}}>
  ...
  const confimUpdate =  <span className={'my-custom-center'} style={{marginRight:'5px'}}>
    ...
    const assess =  <span className={'my-custom-center'}>
  ...
  const returnCancel =  <span className={'my-custom-center'}>
  ...
  const payment = <span style={{marginBottom:'3px'}} className={'my-custom-center'} style={{marginRight:'5px'}}>
  ...
  const updateOrderBack = <span style={{marginBottom:'3px'}} className={'my-custom-center'}>
