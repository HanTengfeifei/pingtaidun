    ...
    if(cloumn=='final_num'){
      for(var i = 0;i<yundans.length;i++){
        ...
        for(var j=0;j<allcars.length;j++){
          var final_num = allcars[j].final_num ?allcars[j].final_num : 0;
          allgoodsnums =allgoodsnums + Number(final_num) ; //结算单采购量
          currentnum = currentnum +　Number(final_num)　;//运单结算量
        }
      ...
      var money = gprice*allgoodsnums+dfee+efee ;
      joptions.goods_num = allgoodsnums ;
      joptions.money = money ; //获取结算金额
      pthis.setState({
          ...
          <Row className={'offline-table-other-line'} type="flex" align="middle">
            ...
            <Col span={6}>
              <div style={{textAlign:'center'}}>
                <a href={'javascript:void(0)'} style={{marginRight:'5px'}} onClick={()=>this.updateZcbd.bind(this)(item)}><Icon type="arrow-up" />上传装车磅单</a>
                <a href={'javascript:void(0)'} style={{marginRight:'5px'}} onClick={()=>this.updateXcbd.bind(this)(item)}><Icon type="arrow-up" />上传卸车磅单</a>
                <a href={'javascript:void(0)'} style={{marginRight:'5px'}} onClick={()=>this.deleteCars.bind(this)(item)}><Icon type="delete" />删除</a>
              </div >
            </Col>
          </Row>
        </div>
