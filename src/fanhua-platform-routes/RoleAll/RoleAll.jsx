...
class RoleAll extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      ids:[{ key: '4690868395292067',label:"wo"},{ key: '4690868395390986',label:"ta"}],
    };
  ...
  render() {
      ...
      render: (text, record) => (
        <span >
          <a  onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            var auth_ids = record.auth_ids ;
            const ids=record.auth_ids.map((item)=>{
              const obj ={};
              obj.key=item.id;
              obj.label=item.name;
                return obj;
          ...
          }}  href="" style={{marginRight:'10px'}}><Icon type="edit" /><span>编辑</span></a>
          <a href="" onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
          ...
          <a href="" style={{marginLeft: 10}}><Icon type="delete" /><span onClick= {(e) => {
            ...
            Modal.confirm({
              ...
              onOk() {
                RequireUtils.baseRequire('role/del-role',{
                  id: record.id,
                },function (data) {
                  if(data.code==1){
                    alert(data.msg);
    ...
    return (
      <div>
        ...
        >
          <EditRole   ref='editRole'  ids={this.state.ids}  record={this.state.record} onRefS={this.onRefS}   go={this.findRoleAll} />
        </Modal>
        <Row gutter={8} style={{ marginTop: '10px' }}>
          <Col span={24} >
            ...
            >
              <AddRole onRef={this.onRef} go={this.findRoleAll} />
            </Modal>
          </Col>
        </Row>
