...
function submit(e) {
  ...
  this.props.form.validateFields((err, values) => {
    ...
    if (err) {
      ...
      var {form} = this.state ;
      var params = values ;
      params.addr_province = form.addr_province ;
      params.addr_city =  form.addr_city ;
      params.addr_county = form.addr_county ;
      RequireUtils.baseRequire('addr/add-addr',params,function (data) {
        if (data.code)
        {
          alert(data.msg);
          this.setState({
...
class AddFieldStationList extends React.Component {
  ...
  companySelect(value){
     RequireUtils.baseRequire('/addr/user-select',{company_id:value},function (data) {
        if(data.code==1) {
  ...
  componentDidMount() {
    ...
    RequireUtils.baseRequire('/addr/company-select',{},function (data) {
       if(data.code==1) {
         var list = data.data.list ;
         this.setState({
           companyoptions:list
  ...
  render() {
    ...
    const {options,linpeopleoptions,companyoptions} = this.state ;
