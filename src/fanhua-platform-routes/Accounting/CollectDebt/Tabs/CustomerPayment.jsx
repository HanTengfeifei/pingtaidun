...
function submit(e) {
  ...
  fileList.forEach((file) => {
    formData.append('RelationForm[img][]', file);
  });
  e.preventDefault();
...
class CustomerPrePayment extends React.Component {
  ...
  componentDidMount() {
    var _this = this ;
    RequireUtils.baseRequire('/relation/supp-select',{},function (data) {
      if(data.code==1) {
    ...
    return (
      <div className={'my-custom-padding'}>
        <Row gutter={8} style={{ marginTop: '10px' }}>
          <Col span={12} >
            <span className={'my-customer-color'}> 本功能为手动添加供应商相关账号</span>
          </Col>
          <Col span={12} >
        ...
        <Row gutter={8} style={{ marginTop: '10px' }}>
          <Col span={24} >
            <p style={{marginBottom:'3px'}}>上传凭证</p>
            <Upload
              {...props}
        ...
        <Row gutter={8} style={{ marginTop: '10px' }}>
          <Col span={6} >
            <Button  type="primary" loading={this.state.loading} onClick={(e)=>submit.bind(this)(e)}>提交</Button>
          </Col>
        </Row>
      </div >
    )
