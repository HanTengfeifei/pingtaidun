...
function onChange(status,id) {
  console.log(status);
  var _this = this ;
  RequireUtils.baseRequire( "platform/forbid-platform",{id:id},function (data) {
...
class EmployeeManagementTable extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      saleadd:false,
      data: [],
      pagination: {pageSize:10,page:1,limit:10,current:1},
      loading: false,
  ...
  fetch(params = {}) {
    getTableData.bind(this)('platform/platform-list',params);
  }
  fetchS(){
    const pager = this.state.pagination;
    this.fetch({
  ...
  componentDidMount() {
    this.fetch({page:1,limit:this.state.pageSize,});
  }
  handleSubmit(e){
    e.preventDefault();
  ...
  deleteStaion(e,id){
    ...
    Modal.confirm({
      ...
      content: '删除后，该账户将被清除',
      onOk() {
        RequireUtils.baseRequire('platform/del-platform',{
          id: id,
        },function (data) {
          message.success(data.msg);
    ...
    const columns = [{
    ...
    },
      {
        ...
        render: (text, record) => {
          ...
          return <span>
                  <a href="javaScript:void(0)" style={{marginRight:'10px'}}><Switch defaultChecked={isCheck} checked={isCheck} onChange={(checked)=>(onChange.bind(this)(record.status,record.id))}/>&nbsp;&nbsp;{mytext}</a>
    ...
    return (
      <div className={'my-table-title-center'}>
        ...
        <Modal title="新增账号"
               ...
               footer={null}
        >
          <AddForm ref={'AddForm'} companyoption={companyoption} productoption={productoption} handleCancel={this.handleSecondCancel} go={this.fetchS}/>
        </Modal>
        {/*<Modal title="账号编辑"*/}
               {/*visible={update}*/}
               {/*onOk={this.handleOk}*/}
