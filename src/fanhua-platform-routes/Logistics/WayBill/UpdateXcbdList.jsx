 ...
 */
function onCancel() {
  this.props.handleUpdateXcbdCancel() ;
...
function submit(e,item) {
  ...
  this.props.form.validateFields((err, values) => {
    ...
    if (err) {
      ...
      formData.append('recv_num',values['recv_num']) ;
...
class zdbdList extends React.Component {
  ...
  render() {
    ...
    const {rimgFile} = this.props ; //rimgFile

    if(this.state.mymark){
      this.state.showFileList = rimgFile ;
    }
